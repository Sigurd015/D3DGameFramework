...
void RecalculateProjection(SceneCamera* out)
{
	if (out->Spec.ProjectionType == ProjectionType::Orthographic)
	{
		...
		out->Projection = DirectX::XMMatrixOrthographicLH(-out->Spec.OrthographicSize * out->Spec.AspectRatio * 0.5f,
			-out->Spec.OrthographicSize * 0.5f, out->Spec.OrthographicNear, out->Spec.OrthographicFar);
	}
}
